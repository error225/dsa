...
Title: Grpah: Minimum Spaning Tree.
Problem Statement:
Represent a graph of your college campus using adjacency list /adjacency matrix. Nodes
should
represent the various departments/institutes and links should represent the distance
between them.
...
public:
    ...
    void prims()
    {
        ...
        v_array[0] = true; // Start from the first department
        ...
        while (n_edges < n - 1)
        {
            ...
            {
                if (v_array[i])
                { // Only consider vertices already included in MST
                    for (int j = 0; j < n; j++)
                    {
